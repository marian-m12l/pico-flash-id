        ...
        if (*((uint32_t*)sfdp_out) == SFDP_MAGIC) {
            ...
            for (int param_idx=0; param_idx<=sfdp_out[6]; param_idx++) {
                ...
                if (sfdp_out[8*(param_idx+1)] == 0) {
                    ...
                    uint8_t address_bytes = (dword_1 & 0x00060000) >> 17;
                    ...
                    printf("SFDP JEDEC DWORD #1: Address bytes: %s\n", address_bytes == 0 ? "3-Bytes addressing only" : (address_bytes == 1 ? "3- or 4-Bytes addressing" : "4-Bytes addressing only"));
